                        ...
                        <form method="GET" action="{{ route('history.index') }}">
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 items-end">
                                ...
                                <div class="flex space-x-2">
                                    <button type="submit"
                                        class="w-full inline-flex items-center justify-center px-4 py-2 bg-primary-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-primary-500">Filter</button>
                                    <a href="{{ route('history.index') }}"
                                        class="w-full inline-flex items-center justify-center px-4 py-2 bg-white dark:bg-gray-600 border border-gray-300 dark:border-gray-500 rounded-md font-semibold text-xs text-gray-700 dark:text-gray-300 uppercase tracking-widest shadow-sm hover:bg-gray-50 dark:hover:bg-gray-500">Reset</a>
                                </div>
                                <div class="sm:col-start-2 md:col-start-auto">
                                    <a href="{{ route('history.export', request()->query()) }}"
                                        class="w-full inline-flex items-center justify-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-500">Export
                                        Excel</a>
                                </div>
                            </div>
                        </form>
                    ...
                    <div class="block md:hidden space-y-4">
                        @forelse ($histories as $history)
                            <div class="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg shadow">
                                <div class="flex justify-between items-start">
                                    <div>
                                        <p class="font-semibold text-gray-900 dark:text-gray-100">
                                            {{ $history->customer_name }}</p>
                                        <p class="text-xs text-gray-500 dark:text-gray-400">
                                            {{ $history->date->format('d M Y, H:i') }}</p>
                                    </div>
                                ...
                                <p class="mt-2 text-sm text-gray-700 dark:text-gray-300">{{ $history->description }}
                                </p>
                            </div>
                        @empty
